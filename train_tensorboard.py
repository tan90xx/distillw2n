...
from utils.utils_env import AttrDict, build_env
from dataset.hubertdataset import MelDataset, mel_spectrogram, get_dataset_filelist, MAX_WAV_VALUE

from models.bigvgan import BigVGAN
from models.soundstream import SoundStream
from models.ddsp import SawSinSub
from models.distillw2n_160 import DistillW2N
...
from utils.utils_bigvgan import (
    plot_spectrogram,
...
def train(rank, a, h):
    ...
    if h.model == "bigvgan":
        ...
        CODEC, F0, SPK, UNIT = False, False, False, False
    elif h.model == "soundstream":
        generator = SoundStream(h).to(device)
        CODEC, F0, SPK, UNIT = True, False, False, False
    elif h.model == "sawsinsub":
        generator = SawSinSub(h).to(device)
        CODEC, F0, SPK, UNIT = False, True, False, False
    elif h.model == "distillw2n":
        generator = DistillW2N(h).to(device)
        CODEC, F0, SPK, UNIT = False, True, True, True
    ...
    def validate(rank, a, h, loader, mode="seen"):
        ...
        with torch.no_grad():
            ...
            for j, batch in enumerate(tqdm(loader)):
                x, y, u, f0, s, y_mel = batch
                x = x.to(device)
                y = y.to(device)
                u = u.to(device)
                s = s.to(device)
                f0 = f0.to(device)
                input_dict = {"x": x, "y": y, "u": u, "f0": f0, "s": s}
                if hasattr(generator, "module"):
                    output_dict = generator.module(input_dict)
                else:
                    output_dict = generator(input_dict)
                ...
                if F0:
                    f0_hat = output_dict["f0"]
                    min_f0t = min(f0.size(-1), f0_hat.size(-1))
                    val_err_tot += F.l1_loss(f0_hat[...,:min_f0t], f0[...,:min_f0t]).item()
                ...
                if j % a.eval_subsample == 0:  # Subsample every nth from validation set
                    ...
                    spec_delta = torch.clamp(
                        torch.abs(y_mel[0].cpu() - y_hat_spec.squeeze(0).cpu()),
                        min=1e-6,
                        max=1.0,
                    )
                    sw.add_figure(
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            y_g_hat = output_dict["y_g_hat"]
            if CODEC:
                loss_codebook = output_dict["codebook_loss"]
            y_g_hat_mel = mel_spectrogram(
                y_g_hat.squeeze(1),
                h.n_fft,
                h.num_mels,
            ...
            if F0:
                f0_hat = output_dict["f0"]
                if f0_hat.numel() == 0:
                    f0_hat = torch.zeros_like(f0)
                min_f0t = min(f0.size(-1), f0_hat.size(-1))
                loss_f0 = F.smooth_l1_loss(f0_hat[...,:min_f0t], f0[...,:min_f0t])
            if UNIT:
                unit_hat = output_dict["u"]
                unit = input_dict["u"]
                min_ut = min(unit.size(-1), unit_hat.size(-1))
                loss_unit = t_axis_distill_loss(unit_hat[...,:min_ut], unit[...,:min_ut])
            ...
            if steps >= a.freeze_step:
                ...
                if CODEC:
                    loss_gen_all += loss_codebook
                if F0:
                    loss_gen_all += loss_f0
                if UNIT:
                    loss_gen_all += loss_unit
            else:
                print(
            ...
            if total_norm > 1000:
                grad_norm_g = torch.nn.utils.clip_grad_norm_(generator.parameters(), 1)
            else:
            ...
            if rank == 0:
                ...
                if steps % a.stdout_interval == 0:
                    ...
                    print(
                        ...
                        + (f"Unit Error: {loss_unit.item():4.3f}, " if UNIT else "")
                        + (f"F0 Error: {loss_f0.item():4.3f}, " if F0 else "")
                        + (f"Codec Error: {loss_codebook.item():4.3f}, " if CODEC else "")
